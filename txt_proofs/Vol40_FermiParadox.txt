...
/-- A chain survives iff-style: if every stage passes, the chain passes. -/
theorem all_pass_survives (c : Chain) (h : ∀ f ∈ c, f = true) :
    passesAll c = true := by
  induction c with
  | nil => rfl
  | cons hd tl ih =>
    simp only [passesAll]
    rw [h hd (List.mem_cons_self hd tl)]
    rw [ih (fun f hf => h f (List.mem_cons_of_mem hd hf))]
    simp
