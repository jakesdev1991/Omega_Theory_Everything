...
fig.suptitle(
    "Ω PROTOCOL v3.6: EMERGENT GRAVITY", color="#00ffff", fontsize=18, fontweight="bold"
)

vel_history: list[float] = []
time_history: list[float] = []
shred_events: list[int] = []
...
def update(frame):
    ...
    # Dynamic Title
    ax1.set_title(
        f"Frame {frame} │ Particles: {len(positions)} │ Shredded: {len(shred_events)}",
        color="#00ffff",
        fontsize=12,
        pad=10,
    )
    ...
    if bh_start >= 0 and bh_end < len(physical_x):
        ax1.axvspan(
            physical_x[int(bh_start)],
            physical_x[int(bh_end)],
            color="red",
            alpha=0.3,
            label="Event Horizon",
        )
    ...
    ax2.plot(
        time_history, vel_history, color="#ffff00", lw=2, label="Inertial Velocity"
    )
